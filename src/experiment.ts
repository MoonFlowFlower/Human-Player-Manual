...
export function experimentHTML(hidden=false): string {
 return `<div class="experiment"><div class="experiment-top"><span class="eyebrow">INTERACTIVE MODEL / 01</span><span class="evidence-badge">MODEL · 教学假设</span></div><h3>改变一个环节，<br>整个结果会怎样？</h3><p>把基准成本指数设为 100，其中运输占 10。只改变运输成本，其他条件保持不变。</p>
 ...
 <div class="experiment-result"><span>模型输出<br><small>基准指数 100</small></span><output id="model-result" aria-live="polite"><b id="model-value">${hidden?'?':'105.0'}</b><span id="model-change">${hidden?'先预测，再揭示':'↑ 5.0%'}</span></output></div>
 <p class="model-formula mono">100 + 10 × 成本变化率 × 传递比例</p><details class="model-limits"><summary>这个模型没有告诉你什么 ${icon('plus',14)}</summary><p>所有份额均为自编教学假设，不是市场调查或 USDA 数据。线性传递省略了需求变化、库存、替代选择、合同、利润变化和时间滞后。它只能检验你是否理解“局部变化不等于整体同比变化”，不能预测真实面包价格。</p></details></div>`;
}
export function mountExperiment(root: HTMLElement,signal: AbortSignal,hidden=false,onChange?:()=>void): {values:()=>[number,number]; reveal:()=>void} {
 ...
 const update=()=>{
  ...
  must(root,'#model-change').textContent=revealed?`↑ ${(out-100).toFixed(1)}%`:'先预测，再揭示';
 };
