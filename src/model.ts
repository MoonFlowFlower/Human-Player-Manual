...
export function canComplete(id: string, completed: string[]): boolean {
  const i = questOrder.indexOf(id);
  return i >= 0 && questOrder.slice(0,i).every(q => completed.includes(q));
}
...
export function unlocks(id: string): Concept[] { return concepts.filter(c => c.prerequisites.includes(id)); }
export function searchContent(query: string): SearchResult[] {
  ...
  }).filter(item=>item.score>0).sort((a,b)=>b.score-a.score).map(({recipe:r}) => ({kind:'recipe' as const,id:r.id,title:r.title,sub:'现实问题 · 查看关系配方'}));
  const conceptResults = concepts.filter(c => !q || `${c.title} ${c.en} ${c.summary}`.toLocaleLowerCase().includes(q)).map(c => ({kind:'concept' as const,id:c.id,title:c.title,sub:`${c.en} · 系统说明书`}));
  return [...recipeResults,...conceptResults].slice(0,14);
}
export function validateContent(): string[] {
  ...
  for (const c of concepts) {
    for (const prior of c.prerequisites) if(!ids.has(prior) || prior===c.id) errors.push(`Invalid prerequisite: ${c.id}/${prior}`);
  ...
  const walk=(id:string):void=>{ if(visiting.has(id)) { errors.push(`Prerequisite cycle at ${id}`); return; } if(visited.has(id)) return; visiting.add(id); for(const p of conceptIndex.get(id)?.prerequisites??[]) walk(p); visiting.delete(id);visited.add(id); };
