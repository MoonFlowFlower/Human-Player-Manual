...
import { conceptIndex, connected, unlocks, safeText } from './model.js';
...
function atlasSVG(state: AppState): string {
 ...
 return `<svg id="atlas-svg" viewBox="0 0 930 610" aria-label="现实系统关系图。Tab 访问节点，回车选择。" xmlns="http://www.w3.org/2000/svg">
  ...
  <g id="graph-transform" transform="translate(${s.dx},${s.dy}) translate(465 305) scale(${s.scale}) translate(-465 -305)">
    <g class="graph-coordinates"><path d="M70 70h790v480H70Z"/><text x="82" y="88">KNOWLEDGE COORDINATES / MODEL 01</text><text x="800" y="535">N ↑</text></g>
...
function nodePanel(state: AppState): string {
 const c=conceptIndex.get(state.atlas.selected)!; const relations=connected(c.id); const next=unlocks(c.id);
 return `<div class="panel-heading"><span class="eyebrow">SYSTEM / ${c.index}</span><span class="status-label ${state.progress.visited.includes(c.id)?'explored':''}">${statusText(state.progress,c.id)}</span></div>
   ...
   <div class="panel-section"><h3>先理解什么 <span>LEARNING PREREQUISITES</span></h3>${c.prerequisites.length?`<div class="node-link-list">${c.prerequisites.map(nodeLink).join('')}</div>`:'<p class="small muted">可直接从这里开始。</p>'}</div>
   <div class="panel-section"><h3>它与什么相连 <span>${relations.length} LINKS</span></h3><div class="panel-relations">${relations.slice(0,4).map(e=>{const other=conceptIndex.get(e.from===c.id?e.to:e.from)!;return `<button data-select="${other.id}"><span>${e.from===c.id?'→':'←'} ${safeText(other.title)}</span><small>${safeText(e.label)}</small></button>`;}).join('')}</div></div>
   ${next.length?`<p class="panel-next">继续理解 ${next.slice(0,2).map(x=>safeText(x.title)).join('、')} ${icon('arrow',14)}</p>`:''}
   <a class="button primary full" href="#guide/${c.id}">打开系统说明 ${icon('arrow')}</a><p class="panel-note">阅读不会自动记为“已掌握”。</p>`;
}
function mobileExplorer(state: AppState): string {
 const c=conceptIndex.get(state.atlas.selected)!;
 return `<div class="mobile-focus"><span class="eyebrow">FOCUSED NODE / ${c.index}</span><div class="focus-symbol">${icon(c.icon,42)}</div><strong>${safeText(c.title)}</strong><span class="mobile-focus-en">${safeText(c.en)}</span><a class="text-link mobile-read" href="#guide/${c.id}">阅读这个系统 ${icon('arrow',16)}</a></div><div class="mobile-links">${connected(c.id).map(e=>{const other=conceptIndex.get(e.from===c.id?e.to:e.from)!;return `<button data-select="${other.id}">${icon(other.icon,20)}<span><b>${safeText(other.title)}</b><small>${e.from===c.id?'向外连接':'来自上游'} · ${safeText(e.label)}</small></span>${icon('arrow',16)}</button>`;}).join('')}</div><label class="mobile-system-select">跳到任一系统<select id="mobile-node-select">${concepts.map(n=>`<option value="${n.id}" ${n.id===c.id?'selected':''}>${n.index} · ${n.title}</option>`).join('')}</select></label>`;
}
export function atlasPage(state: AppState): string {
 const recipe=recipes.find(r=>r.id===state.atlas.recipe);
 return `<main id="main" class="atlas-page page" tabindex="-1">${pageHeader('WORLD ATLAS / 现实系统图集','看见隐藏的连接。','没有孤立的知识。<br>选一个节点，顺着关系探索。')}
  ...
  <div class="atlas-shell"><div class="map-side"><div class="atlas-filters" aria-label="高亮系统分类"><button data-category="all" class="${state.atlas.category==='all'?'active':''}" aria-pressed="${state.atlas.category==='all'}">全部系统 <span>12</span></button>${categories.map(c=>`<button data-category="${c.id}" class="${state.atlas.category===c.id?'active':''}" aria-pressed="${state.atlas.category===c.id}">${c.title}</button>`).join('')}</div>
    <div class="desktop-map" tabindex="0" role="region" aria-label="可移动系统地图，方向键平移，加减键缩放"><div id="graph-container">${atlasSVG(state)}</div><div class="map-tools"><button data-zoom="out" aria-label="缩小地图">${icon('minus',18)}</button><span id="zoom-value" class="mono">${Math.round(state.atlas.scale*100)}%</span><button data-zoom="in" aria-label="放大地图">${icon('plus',18)}</button><button data-zoom="reset" aria-label="复位地图">${icon('reset',18)}</button></div><span class="map-instruction mono">DRAG TO EXPLORE · SELECT TO CONNECT</span></div>
    <div class="mobile-node-explorer">${mobileExplorer(state)}</div>
    <div class="map-legend"><span><i class="line-sample flow"></i>物品流动</span><span><i class="line-sample support"></i>支持关系</span><span><i class="line-sample feedback"></i>可能的反馈</span><span class="legend-model">MODEL · 非实际溯源</span></div>
  </div><aside class="node-panel" aria-label="当前系统详情">${nodePanel(state)}</aside></div>
  ${recipe?`<div class="recipe-detail"><div><span class="eyebrow">RECIPE / ${safeText(recipe.en)}</span><h2>${safeText(recipe.title)}</h2><p>${safeText(recipe.outcome)}</p></div><div class="recipe-chain">${recipe.nodes.map((id,i)=>`${i?(recipe.relationMode==='flow'?'<span class="chain-arrow">→</span>':'<span class="context-separator" aria-hidden="true">·</span>'):''}${nodeLink(id)}`).join('')}${recipe.relationMode==='context'?'<p class="recipe-context-note">共同帮助理解这个问题，不表示单一因果链。</p>':''}</div><a class="text-link" href="#quests/trace">开始现场任务 ${icon('arrow',16)}</a></div>`:''}
  <details class="text-graph"><summary>查看文字版关系与阅读说明 ${icon('plus',14)}</summary><p>箭头表示物品、支持或可能的反馈；不是“先学后学”的强制顺序。实线也不代表完整的真实供应链。前置理解单独列在系统说明里，所有条目始终可读。</p><div>${edges.map(e=>`<p>${nodeLink(e.from)} → ${nodeLink(e.to)}<span>${safeText(e.label)} · ${e.type==='flow'?'物品流动':e.type==='support'?'支持关系':'可能的反馈'}</span></p>`).join('')}</div></details>
 </main>`;
}
